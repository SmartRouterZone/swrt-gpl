...
#if defined(RTCONFIG_RALINK) && defined(RTCONFIG_RALINK_MT7621)
	flood = "-x";
#else
...
void start_lan(void)
{
	...
	char word[256], *next;
	int match;
	char tmp[256], prefix[] = "wlXXXXXXXXXXXXXX";
	int i;
	...
	if (strncmp(lan_ifname, "br", 2) == 0) {
		...
		if ((lan_ifnames = strdup(nvram_safe_get("lan_ifnames"))) != NULL) {
			...
			while ((ifname = strsep(&p, " ")) != NULL) {
				...
				hostapd_add_ra(ifname);
#endif
#elif defined(RTCONFIG_QCA)
				wlconf_qca(ifname);
#elif defined(RTCONFIG_LANTIQ)
				wlconf_lantiq(ifname);
#endif
 ...
 && !defined(RTCONFIG_SWITCH_QCA8075_QCA8337_PHY_AQR107_AR8035_QCA8033)
		...
		if (memcmp(hwaddr, "\0\0\0\0\0\0", ETHER_ADDR_LEN)) {
			...
			ifr.ifr_hwaddr.sa_family = ARPHRD_ETHER;
#if (defined(RTCONFIG_QCA) || defined(RTCONFIG_LANTIQ)) && defined(RTCONFIG_WIRELESSREPEATER) && !(defined(RTCONFIG_CONCURRENTREPEATER) || defined(RTCONFIG_REPEATER_STAALLBAND))
			if (sw_mode() == SW_MODE_REPEATER) {
				char tmpmac[512];
				char *stamac;
 ...
 || defined(RTCONFIG_LANTIQ)
	...
	if (strcmp(lan_subnet, cap_subnet))
		eval("ebtables", "-t", "broute", add_del, "BROUTING", "-i", lifname,
			"-p", "ipv4", "--ip-proto", "icmp", "--ip-dst", cap_subnet, "-j", "DROP");
...
#endif
		eval("ebtables", "-t", "broute", add_del, "BROUTING", "-i", lifname,
			"-p", "ipv4", "--ip-dst", lan_subnet, "--ip-proto", "tcp", "-j", "DROP");
	}
	...
	eval("ebtables", "-t", "broute", add_del, "BROUTING", "-i", lifname, "-p", "ipv4", "--ip-dst", lan_subnet, "-j", "SKIPLOG");
#endif
}
